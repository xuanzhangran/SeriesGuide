...
@implementation JDPlayerController 


- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.view.backgroundColor = SGColorF6F6F;
    //设置导航栏title
    self.navigationItem.title = @"餐厅主题音乐";
...
-(void)setupSubview{
    ...
    _playControllerView.backgroundColor = SGColorOrange;
    ...
    [_playControllerView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(self.view.mas_bottom).with.offset(-49-23);
...
{
    CGFloat kMargin = 10;
    return CGSizeMake((SCREEN_WIDTH - 5*kMargin)/4, 40);
}
